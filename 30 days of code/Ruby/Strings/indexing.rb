...
# The string class provides a convenient array-like square bracket [] operator, 
...
# The first operand specifies an index (which may be negative), and the second specifies a length (which must be non-negative).
...
# "!"
# The same examples shown above can be used for assignment / deletion of characters. 
# We can insert characters by giving a non-empty assignment or delete characters in the range by giving an empty assignment.
...
# But wait, there's more! Ruby also allows you to index strings using a Range or a Regexp object as well. 
# We will discuss these methods in the future.
